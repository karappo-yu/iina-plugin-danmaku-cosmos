...
function findDanmakuByEpisode(videoUrl) {
  ...
  var cacheDir = '';
  try {
    cacheDir = iina.utils.resolvePath('@data/danmaku-cache/');
    if (cacheDir && file.exists(cacheDir)) searchDirs.push(cacheDir);
  } catch (e) {}
  ...
  for (var d = 0; d < searchDirs.length; d++) {
    ...
    for (var j = 0; j < items.length; j++) {
      ...
      if (ext === 'json' && dir === cacheDir) {
        try {
          var content = file.read(filePath);
          if (content && content.trim().charAt(0) === '{') {
            var obj = JSON.parse(content);
            if (obj.source === 'dandanplay' && obj.comments) {
              fileInfo.type = 'dandanplay';
              fileInfo.filename = '🌐 ' + (obj.animeTitle || 'DanDanPlay') + ' - ' + (obj.episodeTitle || '') + '.json';
            }
          }
        } catch (e) {}
      }
...
function md5(input) {
  ...
  function core(a, b, c, d, x) {
    ...
    return [safeAdd(a, oa), safeAdd(b, ob), safeAdd(c, oc), safeAdd(d, od)];
  }
  function bytesToWords(bytes, offset, len) {
    var words = [];
    for (var i = 0; i < len; i += 4) {
      words[i >> 2] = (bytes[offset + i] & 0xFF) |
        ((bytes[offset + i + 1] & 0xFF) << 8) |
        ((bytes[offset + i + 2] & 0xFF) << 16) |
        ((bytes[offset + i + 3] & 0xFF) << 24);
    }
    return words;
  }
  function hexStr(a, b, c, d) {
    var hexTab = '0123456789abcdef';
    var str = '';
    var arr = [a, b, c, d];
    for (var j = 0; j < 4; j++) {
      for (var i = 0; i < 4; i++) {
        str += hexTab.charAt((arr[j] >>> (i * 8 + 4)) & 0xF) +
               hexTab.charAt((arr[j] >>> (i * 8)) & 0xF);
      }
    }
    return str;
  }
  ...
  function processChunk(chunk) {
    var offset = 0;
    if (pendingLen > 0) {
      var need = 64 - pendingLen;
      if (chunk.length < need) {
        pending.set(chunk, pendingLen);
        pendingLen += chunk.length;
        totalLen += chunk.length;
        return;
      }
      pending.set(chunk.subarray(0, need), pendingLen);
      var w = bytesToWords(pending, 0, 64);
      var r = core(state[0], state[1], state[2], state[3], w);
      state[0] = r[0]; state[1] = r[1]; state[2] = r[2]; state[3] = r[3];
      offset = need;
      pendingLen = 0;
    }
    while (offset + 64 <= chunk.length) {
      var w = bytesToWords(chunk, offset, 64);
      var r = core(state[0], state[1], state[2], state[3], w);
      state[0] = r[0]; state[1] = r[1]; state[2] = r[2]; state[3] = r[3];
      offset += 64;
    }
    if (offset < chunk.length) {
      if (!pending) pending = new Uint8Array(64);
      pending.set(chunk.subarray(offset), 0);
      pendingLen = chunk.length - offset;
    }
    totalLen += chunk.length;
  }
  function digest() {
    var bitLen = totalLen * 8;
    var padLen = 64 - pendingLen;
    if (padLen < 9) padLen += 64;
    var padBuf = new Uint8Array(64);
    padBuf[0] = 0x80;
    processChunk(padBuf.subarray(0, padLen - 8));
    var lenBuf = new Uint8Array(8);
    lenBuf[0] = bitLen & 0xFF;
    lenBuf[1] = (bitLen >>> 8) & 0xFF;
    lenBuf[2] = (bitLen >>> 16) & 0xFF;
    lenBuf[3] = (bitLen >>> 24) & 0xFF;
    processChunk(lenBuf);
    return hexStr(state[0], state[1], state[2], state[3]);
  }
  if (input && input.length > 0) processChunk(input);
  return { update: processChunk, digest: digest };
}

function ddpCalcFileHash(filePath) {
  if (!filePath) return Promise.resolve(null);
  try {
    var h = file.handle(filePath, 'read');
    var m = md5(null);
    var chunkSize = 4 * 1024 * 1024;
    var totalRead = 0;
    var limit = 16 * 1024 * 1024;
    while (totalRead < limit) {
      var toRead = Math.min(chunkSize, limit - totalRead);
      var chunk = h.read(toRead);
      if (!chunk || chunk.length === 0) break;
      m.update(chunk);
      totalRead += chunk.length;
    }
    h.close();
    if (totalRead === 0) return Promise.resolve(null);
    var hash = m.digest();
    var fileSize = 0;
    try { fileSize = mpv.getNumber("file-size") || 0; } catch(e) {}
    console.log('[ddp] hash: md5=' + hash + ' fileSize=' + fileSize + ' read=' + totalRead);
    return Promise.resolve({ hash: hash, fileSize: fileSize });
  } catch(e) {
...
function ddpSaveLocalCache(videoPath, episodeId, animeTitle, episodeTitle, converted) {
  try {
    ...
    file.write('@data/danmaku-cache/' + baseName + '.json', JSON.stringify(cacheData));
    console.log('[ddp] saved local cache: ' + baseName + '.json');
  } catch (e) {
    console.log('[ddp] save local cache error: ' + e);
  }
}
...
function ddpAddToFileListAndLoad(episodeId, animeTitle, episodeTitle, converted, forceLoad) {
  ...
  ddpSaveLocalCache(currentVideoUrl || '', episodeId, animeTitle, episodeTitle, converted);
...
function loadDanmakuForVideo(url) {
  ...
  var cachedDDPFile = null;
  for (var i = 0; i < danmakuFileList.jsonFiles.length; i++) {
    if (danmakuFileList.jsonFiles[i].type === 'dandanplay') { cachedDDPFile = danmakuFileList.jsonFiles[i]; break; }
  }

  if (dandanplayPriority === 'network-first') {
    if (cachedDDPFile) {
      loadLocalDanmaku(cachedDDPFile);
    } else {
      ddpAutoMatchAndLoad(url);
      if (!hasLocal) danmakuNotFound();
    }
...
function loadLocalDanmaku(fileInfo) {
  ...
  if (fileType === 'dandanplay') {
    try {
      ...
      if (obj.comments) contentToSend = JSON.stringify(obj.comments);
    } catch (e) {}
  }
