...
(function(global, factory) {
	...
	*/
	...
	//#endregion
	//#region node_modules/.pnpm/valibot@1.4.0_typescript@6.0.2/node_modules/valibot/dist/index.mjs
	let store$4;
	...
	*/
	const parseCommandAndNicoScript = (comment) => {
		...
		return {
			...
			reverse: comment.reverse || commands.reverse,
	...
	*/
	const parseCommand = (comment, _command, result, isFlash) => {
		...
		if (command === "reverse" || command === "nico:reverse") {
			result.reverse = true;
			return;
		}
	...
	*/
	...
	const pushTimelineComment = (timeline, vpos, comment, dedupe) => {
		var _timeline$vpos;
		if (dedupe && ((_timeline$vpos = timeline[vpos]) === null || _timeline$vpos === void 0 ? void 0 : _timeline$vpos.includes(comment))) return false;
	...
	*/
	const processFixedComment = (comment, collision, timeline, lazy = false) => {
		...
		for (let j = 0; j < commentLong; j++) {
			...
			if (!pushTimelineComment(timeline, vpos, comment, lazy)) continue;
	...
	*/
	const processMovableComment = (comment, collision, timeline, lazy = false) => {
		...
		const reverse = comment.comment.reverse;
		...
		for (let j = beforeVpos; j < n; j++) {
			const vpos = commentVpos + j;
			const normalLeftPos = drawPadding + drawRange - (j + 100) * speed;
			const leftPos = reverse ? config.canvasWidth - commentWidth - normalLeftPos : normalLeftPos;
			if (!pushTimelineComment(timeline, vpos, comment, lazy)) continue;
...
`;
	var CSSRenderer = class {
		constructor(canvas) {
			_defineProperty(this, "container", void 0);
			_defineProperty(this, "pool", []);
			_defineProperty(this, "activeElements", /* @__PURE__ */ new Map());
			_defineProperty(this, "activeReverseState", /* @__PURE__ */ new Map());
			_defineProperty(this, "activeSeenGeneration", /* @__PURE__ */ new Map());
			_defineProperty(this, "visibilityGeneration", 0);
			_defineProperty(this, "styleElement", void 0);
			_defineProperty(this, "paused", false);
			_defineProperty(this, "lastUpdateVpos", -1);
			_defineProperty(this, "measureCanvas", null);
			_defineProperty(this, "ascentCache", /* @__PURE__ */ new Map());
		...
		pause() {
			...
			for (const [, element] of this.activeElements) {
				...
				for (let i = 0, n = animations.length; i < n; i++) {
					var _animations$i;
					(_animations$i = animations[i]) === null || _animations$i === void 0 || _animations$i.pause();
				}
			}
		}
		resume() {
			...
			for (const [, element] of this.activeElements) {
				...
				for (let i = 0, n = animations.length; i < n; i++) {
					var _animations$i2;
					(_animations$i2 = animations[i]) === null || _animations$i2 === void 0 || _animations$i2.play();
				}
			}
		}
		updateComments(comments, vpos, frameActiveState) {
			if (frameActiveState.banActive) {
				if (this.activeElements.size > 0) this.clear();
				return 0;
			}
			const generation = ++this.visibilityGeneration;
			const isSeek = this.lastUpdateVpos >= 0 && Math.abs(vpos - this.lastUpdateVpos) > 150;
			let drawnCount = 0;
			for (let i = 0, n = comments.length; i < n; i++) {
				const comment = comments[i];
				if (!comment || comment.invisible) continue;
				this.activeSeenGeneration.set(comment.index, generation);
				const reverse = comment.comment.reverse || (comment.owner ? frameActiveState.reverseActiveOwner : frameActiveState.reverseActiveViewer);
				const element = this.activeElements.get(comment.index);
				if (!element) {
					if (!this.createCommentElement(comment, vpos, frameActiveState)) {
						this.activeSeenGeneration.delete(comment.index);
						continue;
					}
				} else if (comment.loc === "naka") {
					if (this.activeReverseState.get(comment.index) !== reverse || isSeek) this.reanimateScroll(comment, vpos, reverse);
				} else if (isSeek) this.setupFixedAnimation(element, comment, vpos, this.getEffectiveAlpha(comment.comment));
				drawnCount++;
			}
			this.lastUpdateVpos = vpos;
			for (const [index, element] of this.activeElements) if (this.activeSeenGeneration.get(index) !== generation) {
		...
		createCommentElement(comment, vpos, frameActiveState) {
			var _ref, _config$fonts$html5$c;
			const element = this.getElementFromPool();
			const c = comment.comment;
			const reverse = comment.comment.reverse || (comment.owner ? frameActiveState.reverseActiveOwner : frameActiveState.reverseActiveViewer);
			const drawScale = getConfig(config.commentScale, comment.flash);
			const layerScale = c.layer === -1 ? options.scale : 1;
			const { fontSize: renderFontSize, scale: fontScale } = getFontSizeAndScale(c.charSize);
			const fontSizePx = renderFontSize * drawScale * fontScale * layerScale;
			const lineHeightPx = c.lineHeight * drawScale * layerScale;
			const paddingTopCanvas = (10 - fontScale * 10) * ((c.lineCount + 1) / config.html5HiResCommentCorrection) * lineHeightPx;
			const fontOffset = (_ref = c.font === "gothic" || c.font === "mincho" || c.font === "defont" ? (_config$fonts$html5$c = config.fonts.html5[c.font]) === null || _config$fonts$html5$c === void 0 ? void 0 : _config$fonts$html5$c.offset : void 0) !== null && _ref !== void 0 ? _ref : 0;
			const offsetYCanvas = ((c.charSize - c.lineHeight) / 2 + c.lineHeight * -.16 + fontOffset) * drawScale * layerScale;
			let posY;
			if (comment.loc === "shita") posY = config.canvasHeight - comment.posY - comment.height;
			...
			const ascentRendered = this.measureAscentFraction(fontFamily, fontWeight) * fontSizePx;
			...
			if (c.fillColor) element.style.backgroundColor = c.fillColor;
			if (c.wakuColor) {
			...
			} else this.setupFixedAnimation(element, comment, vpos, effectiveAlpha);
			this.container.appendChild(element);
			this.activeElements.set(comment.index, element);
			if (comment.loc === "naka") this.activeReverseState.set(comment.index, reverse);
			if (this.paused) {
				const animations = element.getAnimations();
				for (let i = 0, n = animations.length; i < n; i++) {
					var _animations$i3;
					(_animations$i3 = animations[i]) === null || _animations$i3 === void 0 || _animations$i3.pause();
				}
			}
			return true;
		}
		reanimateScroll(comment, vpos, reverse) {
			const element = this.activeElements.get(comment.index);
			if (!element) return;
			const animations = element.getAnimations();
			for (let i = 0, n = animations.length; i < n; i++) {
				var _animations$i4;
				(_animations$i4 = animations[i]) === null || _animations$i4 === void 0 || _animations$i4.cancel();
			}
			if (!this.setupScrollAnimation(element, comment, vpos, reverse)) {
				this.recycleElement(element);
			...
			if (this.paused) {
				...
				for (let i = 0, n = animations.length; i < n; i++) {
					var _animations$i5;
					(_animations$i5 = animations[i]) === null || _animations$i5 === void 0 || _animations$i5.pause();
				}
			}
		}
		setupScrollAnimation(element, comment, vpos, reverse) {
			const c = comment.comment;
			...
			const speed = (config.commentDrawRange + c.width * config.nakaCommentSpeedOffset) / (comment.long + 100);
			const vposLapsed = vpos - comment.vpos;
			const normalXPx = config.commentDrawPadding + config.commentDrawRange - (vposLapsed + 100) * speed;
			let currentXPx;
			let toXPx;
			let remainingPx;
			if (reverse) {
				currentXPx = config.canvasWidth - c.width - normalXPx;
				toXPx = config.canvasWidth + fontSizePx;
				remainingPx = toXPx - currentXPx;
			} else {
				currentXPx = normalXPx;
				toXPx = -(c.width + fontSizePx);
				remainingPx = currentXPx - toXPx;
			}
			const remainingSec = remainingPx / (speed * 100);
			if (remainingSec <= 0) return false;
			element.animate([{ transform: `translateX(calc(${currentXPx} * var(--dm-unit)))` }, { transform: `translateX(calc(${toXPx} * var(--dm-unit)))` }], {
				duration: remainingSec * 1e3,
				easing: "linear",
				fill: "forwards"
			});
			return true;
		}
		setupFixedAnimation(element, comment, vpos, effectiveAlpha) {
		...
		getEffectiveAlpha(c) {
			...
			return c._live ? config.contextFillLiveOpacity : 1;
		}
		measureAscentFraction(fontFamily, fontWeight) {
			var _fontBoundingBoxAscen;
			...
			const fraction = ((_fontBoundingBoxAscen = ctx.measureText("Ag").fontBoundingBoxAscent) !== null && _fontBoundingBoxAscen !== void 0 ? _fontBoundingBoxAscen : measureFontSize * .9) / measureFontSize;
			this.ascentCache.set(cacheKey, fraction);
			return fraction;
		}
		applyFont(element, font, isFlash) {
			if (isFlash) {
				const flashFonts = config.fonts.flash;
				const fontTemplate = flashFonts[font] || flashFonts.gulim;
				if (fontTemplate) {
					const parts = fontTemplate.split("[size]px");
					if (parts.length === 2) {
						var _parts$0$trim, _parts$, _parts$1$trim, _parts$2;
						const before = (_parts$0$trim = (_parts$ = parts[0]) === null || _parts$ === void 0 ? void 0 : _parts$.trim()) !== null && _parts$0$trim !== void 0 ? _parts$0$trim : "";
						const after = (_parts$1$trim = (_parts$2 = parts[1]) === null || _parts$2 === void 0 ? void 0 : _parts$2.trim()) !== null && _parts$1$trim !== void 0 ? _parts$1$trim : "";
						const weightMatch = before.match(/(\d+)\s*$/);
						if (weightMatch === null || weightMatch === void 0 ? void 0 : weightMatch[1]) element.style.fontWeight = weightMatch[1];
						element.style.fontFamily = after;
					}
				}
			} else {
				const html5Fonts = config.fonts.html5;
				const fontConfig = font === "gothic" || font === "mincho" || font === "defont" ? html5Fonts[font] : html5Fonts.defont;
				if (fontConfig) {
		...
		recycleElement(element) {
			...
			for (let i = 0, n = animations.length; i < n; i++) {
				var _animations$i6;
				(_animations$i6 = animations[i]) === null || _animations$i6 === void 0 || _animations$i6.cancel();
			}
			element.remove();
			element.style.cssText = "";
			element.textContent = "";
			if (this.pool.length < POOL_MAX_SIZE) this.pool.push(element);
		}
		clear() {
			for (const [, element] of this.activeElements) this.recycleElement(element);
			this.activeElements.clear();
			this.activeReverseState.clear();
			this.activeSeenGeneration.clear();
			this.lastUpdateVpos = -1;
		}
		getVisibleCommentIndices() {
	...
	var NiconiComments = class {
		...
		*/
		constructor(_renderer, data, initOptions = {}) {
			...
			if (options.mode === "css") this.cssRenderer = new CSSRenderer(this.renderer.canvas);
		...
		*/
		preRendering(_rawData) {
			...
			let instances = [];
			for (let index = 0, n = rawData.length; index < n; index++) {
				const comment = rawData[index];
				if (!comment) continue;
				instances.push(createCommentInstance(comment, this.renderer, index));
			}
		...
		*/
		sortTimelineComment() {
			...
			for (const vpos of Object.keys(this.timeline)) {
				...
				if (!item) continue;
				if (item.length > 1) item.sort((a, b) => Number(a.owner) - Number(b.owner) || a.index - b.index);
			}
		...
		*/
		addComments(...rawComments) {
			resetRangePointers();
			const comments = [];
			for (let index = 0, n = rawComments.length; index < n; index++) {
				const comment = rawComments[index];
				if (!comment) continue;
				comments.push(createCommentInstance(comment, this.renderer, this.comments.length + index));
			}
		...
		*/
		drawCanvas(vpos, forceRendering = false, cursor) {
			...
			if (this.cssRenderer) {
				this.lastVpos = vpos;
				this._resolveCommentPositions(timelineRange, vpos);
		...
		*/
		...
		_resolveCommentPositions(timelineRange, vpos) {
			let startIndex = 0;
			...
			const banActive = isBanActive(vpos);
			if (banActive && !options.lazy) return;
		...
		*/
		clear() {
			if (this.cssRenderer) this.cssRenderer.clear();
			const size = this.renderer.getSize();
			this.renderer.clearRect(0, 0, size.width, size.height);
		}
		pauseCSS() {
			if (this.cssRenderer) this.cssRenderer.pause();
		}
		resumeCSS() {
			if (this.cssRenderer) this.cssRenderer.resume();
		}
		getVisibleComments() {
			...
			for (const idx of indices) {
				...
				result.push({
					...
					mail: c.mail,
					comment: {
						charSize: c.comment.charSize,
						lineHeight: c.comment.lineHeight,
						fontSize: c.comment.fontSize,
						lineCount: c.comment.lineCount,
						width: c.comment.width,
						scale: c.comment.scale,
						resized: c.comment.resized,
						resizedX: c.comment.resizedX,
