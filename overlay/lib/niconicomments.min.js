...
`;
	var CSSRenderer = class {
		...
		createCommentElement(comment, vpos, frameActiveState) {
			...
			if (comment.loc === "naka") this.setupScrollAnimation(element, comment, vpos, reverse);
		...
		setupScrollAnimation(element, comment, vpos, reverse) {
			...
			const vposLapsed = vpos - comment.vpos;
			let currentXPx = config.commentDrawPadding + config.commentDrawRange - (vposLapsed + 100) * speed;
			if (reverse) currentXPx = config.canvasWidth - c.width - currentXPx;
			const fromXPx = config.commentDrawPadding + config.commentDrawRange;
			const toXPx = -c.width;
			const totalDurationSec = (fromXPx - toXPx) / (speed * 100);
			const elapsedSec = (vposLapsed + 100) / 100;
			element.style.transform = `translateX(calc(${currentXPx} * var(--dm-unit)))`;
			const animFromX = reverse ? toXPx : fromXPx;
			const animToX = reverse ? fromXPx : toXPx;
			element.animate([{ transform: `translateX(calc(${animFromX} * var(--dm-unit)))` }, { transform: `translateX(calc(${animToX} * var(--dm-unit)))` }], {
				duration: totalDurationSec * 1e3,
				easing: "linear",
				fill: "forwards",
				delay: -elapsedSec * 1e3
			});
		}
