...
`;
	var CSSRenderer = class {
		...
		getDrawScale(comment) {
			return getConfig(config.commentScale, comment.flash) * (comment.comment.layer === -1 ? options.scale : 1);
		}
		pause() {
		...
		createCommentElement(comment, vpos, frameActiveState) {
			...
			const posY = comment.loc === "shita" ? config.canvasHeight - scaledPosY - scaledHeight : scaledPosY;
