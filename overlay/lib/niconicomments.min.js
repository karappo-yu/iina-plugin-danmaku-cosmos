	...
	var CSSRenderer = class {
		...
		updateComments(comments, vpos, frameActiveState) {
			...
			for (let i = 0, n = comments.length; i < n; i++) {
				...
				if (frameActiveState.banActive) continue;
				currentVisible.add(comment.index);
				const reverse = comment.owner ? frameActiveState.reverseActiveOwner : frameActiveState.reverseActiveViewer;
				if (!this.activeElements.has(comment.index)) this.createCommentElement(comment, vpos, frameActiveState);
				else if (comment.loc === "naka" && this.activeReverseState.get(comment.index) !== reverse) this.reanimateScroll(comment, vpos, reverse);
				drawnCount++;
			}
			for (const [index, element] of this.activeElements) if (!currentVisible.has(index)) {
		...
		createCommentElement(comment, vpos, frameActiveState) {
			...
			if (this.paused) {
				const animations = element.getAnimations();
				for (let i = 0, n = animations.length; i < n; i++) animations[i].pause();
			}
		}
		reanimateScroll(comment, vpos, reverse) {
			...
			for (let i = 0, n = animations.length; i < n; i++) {
				var _animations$i3;
				(_animations$i3 = animations[i]) === null || _animations$i3 === void 0 || _animations$i3.cancel();
			}
			if (!this.setupScrollAnimation(element, comment, vpos, reverse)) {
				this.recycleElement(element);
			...
			if (this.paused) {
				...
				for (let i = 0, n = animations.length; i < n; i++) {
					var _animations$i4;
					(_animations$i4 = animations[i]) === null || _animations$i4 === void 0 || _animations$i4.pause();
				}
			}
		}
			...
			if (isFlash) {
				...
				if (fontTemplate) {
					...
					if (parts.length === 2) {
						...
						if (weightMatch && weightMatch[1]) element.style.fontWeight = weightMatch[1];
		...
		recycleElement(element) {
			...
			for (let i = 0, n = animations.length; i < n; i++) {
				var _animations$i5;
				(_animations$i5 = animations[i]) === null || _animations$i5 === void 0 || _animations$i5.cancel();
			}
			element.remove();
