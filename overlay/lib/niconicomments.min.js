	...
	var HTML5Comment = class extends BaseComment {
		...
		_processResizeX(comment, width, layerScale = 1) {
			...
			if (baseCharSize >= 1) {
				...
				let low = Math.max(1, Math.floor(legacyBaseCharSize * .5));
	...
	var CSSRenderer = class {
		...
		updateComments(comments, vpos, frameActiveState) {
			...
			for (let i = startIndex; i < endIndex; i++) {
				...
				} else if (comment.loc === "naka" && isSeek) this.reanimateScroll(comment, vpos);
		...
		createCommentElement(comment, vpos) {
		...
		setupScrollAnimation(element, comment, vpos) {
			const c = comment.comment;
			...
			const remainingSec = (currentXPx - toXPx) / (speed * 100);
