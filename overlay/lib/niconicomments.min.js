...
`;
	var CSSRenderer = class {
		...
		getDrawScale(comment) {
			return getConfig(config.commentScale, comment.flash);
		}
		pause() {
		...
		updateComments(comments, vpos, frameActiveState) {
			...
			for (let i = 0, n = comments.length; i < n; i++) {
				...
				if (!this.activeElements.get(comment.index)) {
					if (!this.createCommentElement(comment, vpos, frameActiveState)) {
						this.activeSeenGeneration.delete(comment.index);
						continue;
					}
				} else if (comment.loc === "naka" && this.activeReverseState.get(comment.index) !== reverse) this.reanimateScroll(comment, vpos, reverse);
				drawnCount++;
			}
		...
		createCommentElement(comment, vpos, frameActiveState) {
			...
			const effectiveAlpha = typeof c.opacity === "number" ? c.opacity : c._live ? config.contextFillLiveOpacity : 1;
			element.style.top = `calc(${posY} * var(--dm-unit))`;
			element.style.paddingTop = `calc(${paddingTopCanvas + offsetYCanvas} * var(--dm-unit))`;
		...
		setupScrollAnimation(element, comment, vpos, reverse) {
			...
			const fontSizePx = c.fontSize * commentScale;
