...
(function(global, factory) {
	...
	*/
	const isFlashComment = (comment) => options.mode === "flash" || (options.mode === "default" || options.mode === "css") && !(comment.mail.includes("gothic") || comment.mail.includes("defont") || comment.mail.includes("mincho")) && (comment.date < config.flashThreshold || comment.mail.includes("nico:flash"));
	/**
	* コメントが逆コマンド適用対象かを返す
...
`;
	var CSSRenderer = class {
		...
		resume() {
			...
			for (const [, element] of this.activeElements) {
				const animations = element.getAnimations();
				for (let i = 0, n = animations.length; i < n; i++) animations[i].play();
			}
		}
		...
		createCommentElement(comment, vpos, frameActiveState) {
			...
			if (comment.flash) element.style.lineHeight = String(c.lineHeight);
			else element.style.lineHeight = String(c.lineHeight / c.fontSize);
		...
		reanimateScroll(comment, vpos, reverse) {
			...
			for (let i = 0, n = animations.length; i < n; i++) {
				var _animations$i;
				(_animations$i = animations[i]) === null || _animations$i === void 0 || _animations$i.cancel();
			}
			if (!this.setupScrollAnimation(element, comment, vpos, reverse)) {
				this.recycleElement(element);
			...
			if (this.paused) {
				...
				for (let i = 0, n = newAnimations.length; i < n; i++) {
					var _newAnimations$i;
					(_newAnimations$i = newAnimations[i]) === null || _newAnimations$i === void 0 || _newAnimations$i.pause();
				}
			}
		}
			...
			if (isFlash) {
				...
				if (fontTemplate) {
					...
					if (parts.length === 2) {
						...
						if (weightMatch) element.style.fontWeight = weightMatch[1];
		...
		recycleElement(element) {
			const animations = element.getAnimations();
			for (let i = 0, n = animations.length; i < n; i++) animations[i].cancel();
			element.remove();
