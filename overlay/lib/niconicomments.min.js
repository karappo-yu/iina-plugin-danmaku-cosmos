...
(function(global, factory) {
	...
	*/
	...
	const isMode = (i) => i === "default" || i === "html5" || i === "flash" || i === "css";
	...
	*/
	...
	var FlashComment = class extends BaseComment {
		...
		*/
		getCommentSize(parsedData) {
			...
			if (this.ctx.options.scale !== 1 && parsedData.layer === -1 && !parsedData.owner) {
		...
		measureText(comment) {
			...
			const layerScale = comment.layer === -1 && !comment.owner ? this.ctx.options.scale : 1;
			...
			if (showCollision) {
				...
				try {
					...
					for (let i = 0, n = this.comment.lineCount; i < n; i++) {
						...
						this.renderer.strokeRect(posX, posY + linePosY * this._globalScale, this.comment.width, this.comment.fontSize * this.comment.lineHeight * -1 * this._globalScale * this.comment.scale * (this.comment.layer === -1 && !this.comment.owner ? this.ctx.options.scale : 1));
					}
				} finally {
					this.renderer.restore();
				}
		...
		isHovered(_cursor, _posX, _posY) {
			...
			const scaleY = this._globalScale * this.comment.scale * (this.comment.layer === -1 && !this.comment.owner ? this.ctx.options.scale : 1);
		...
		_setupCanvas(renderer) {
			...
			const scale = this._globalScale * this.comment.scale * (this.comment.layer === -1 && !this.comment.owner ? this.ctx.options.scale : 1);
	...
	var HTML5Comment = class extends BaseComment {
		...
		getTextImageBounds() {
			...
			const layerScale = this.comment.layer === -1 && !this.comment.owner ? this.ctx.options.scale : 1;
...
`;
	var CSSRenderer = class {
		constructor(canvas, config, options) {
			...
			_defineProperty(this, "activeElementReverse", /* @__PURE__ */ new Map());
			...
			_defineProperty(this, "playbackSpeed", 1);
		...
		updateComments(comments, vpos, frameActiveState) {
			...
			const excludedByLimit = /* @__PURE__ */ new Set();
			if (limit !== void 0) {
				if (limit === 0) {
					if (this.activeElements.size > 0) this.clear();
				...
				if (this.config.hideCommentOrder === "asc") startIndex = Math.max(0, comments.length - limit);
				else endIndex = Math.min(comments.length, limit);
			...
			if (newElements.length > 0) {
				...
				for (let i = 0, n = newElements.length; i < n; i++) fragment.appendChild(newElements[i]);
		...
		createCommentElement(comment, vpos, isReverse) {
			...
			const rawLayerScale = c.layer === -1 && !comment.owner ? this.options.scale : 1;
			const { fontSize: renderFontSize, scale: fontScale } = getFontSizeAndScale(c.charSize, this.config);
			const layerScale = this.applyScaleFloor(rawLayerScale, renderFontSize * drawScale * fontScale, drawScale);
			const fontSizePx = renderFontSize * drawScale * fontScale * layerScale;
			const lineHeightPx = c.lineHeight * drawScale * layerScale;
			const paddingTopCanvas = (10 - fontScale * 10) * ((c.lineCount + 1) / this.config.html5HiResCommentCorrection) * lineHeightPx;
			const fontOffset = (_ref = c.font === "gothic" || c.font === "mincho" || c.font === "defont" ? (_this$config$fonts$ht = this.config.fonts.html5[c.font]) === null || _this$config$fonts$ht === void 0 ? void 0 : _this$config$fonts$ht.offset : void 0) !== null && _ref !== void 0 ? _ref : 0;
			...
			} else this.setupFixedAnimation(element);
			this.activeElementReverse.set(comment.index, isReverse);
			this.activeElements.set(comment.index, element);
			element.__dmComment = comment;
			return true;
		}
		reanimateScroll(comment, vpos, isReverse) {
			const element = this.activeElements.get(comment.index);
			if (!element) return;
		...
		*/
		applyScaleFloor(layerScale, unscaledFontSizePx, drawScale) {
			if (layerScale >= 1 || unscaledFontSizePx <= 0) return layerScale;
			const mediumFont = getFontSizeAndScale(this.getMediumCharSize(), this.config);
			const minFontSizePx = mediumFont.fontSize * mediumFont.scale * drawScale * .25;
		...
		setupScrollAnimation(element, comment, vpos, isReverse) {
			const c = comment.comment;
			const commentScale = getConfig(this.config.commentScale, comment.flash);
			const rawLayerScale = c.layer === -1 && !comment.owner ? this.options.scale : 1;
			const { fontSize: renderFontSize, scale: fontScale } = getFontSizeAndScale(c.charSize, this.config);
			const layerScale = this.applyScaleFloor(rawLayerScale, renderFontSize * commentScale * fontScale, commentScale);
			const fontSizePx = renderFontSize * commentScale * fontScale * layerScale;
			const speed = (this.config.commentDrawRange + c.width * this.config.nakaCommentSpeedOffset) / (comment.long + 100);
			const vposLapsed = vpos - comment.vpos;
			const currentXPx = this.config.commentDrawPadding + this.config.commentDrawRange - (vposLapsed + 100) * speed;
			const toXPx = -(c.width + fontSizePx);
			const remainingSec = (currentXPx - toXPx) / (speed * 100) / this.playbackSpeed;
		...
		applyFont(element, font, isFlash) {
			if (isFlash) {
				const flashFonts = this.config.fonts.flash;
		...
		getElementFromPool() {
			while (this.pool.length > 0) return this.pool.pop();
	...
	var NiconiComments = class {
		...
		*/
		constructor(_renderer, data, initOptions = {}) {
			...
			if (options.mode === "css" && _renderer instanceof HTMLCanvasElement) this.cssRenderer = new CSSRenderer(this.renderer.canvas, config, options);
		...
		drawCanvas(vpos, forceRendering = false, cursor) {
			...
			if (this.cssRenderer) {
				...
				const frameActiveState = {
					...
					reverseActiveOwner: isReverseActive(vpos, true, this.ctx.nicoScripts, this.ctx.rangeCache),
					reverseActiveViewer: isReverseActive(vpos, false, this.ctx.nicoScripts, this.ctx.rangeCache)
				};
		...
		*/
		_resolveCommentPositions(timelineRange, banActive) {
			const { config } = this.ctx;
			let startIndex = 0;
			let endIndex = timelineRange.length;
			if (config.commentLimit !== void 0) if (config.commentLimit === 0) return;
		...
		*/
		...
		setPlaybackSpeed(speed) {
			if (this.cssRenderer) this.cssRenderer.setPlaybackSpeed(speed);
		}
		getVisibleComments() {
