...
**Manual trigger always loads regardless of the toggle**: the 网络匹配 button (`dandanplay-trigger-match`), candidate selection (`dandanplay-select-match`) and episode selection (`dandanplay-select-episode`) are explicit user actions — they pass `manual=true` through `ddpAutoMatchAndLoad`/`ddpLoadComments`, which bypasses the autoNetwork priority in `forceLoad` and both danmaku-toggle-off abort guards (at match response in `ddpAutoMatchAndLoad` and at download completion in `ddpLoadComments`); loading then re-enables the toggle via `ensureDanmakuEnabled()`, same as manually picking a file from the list. Background auto-match (no `manual`) keeps the toggle/priority semantics unchanged.
